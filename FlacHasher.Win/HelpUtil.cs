...
namespace Andy.FlacHash.Application.Win
{
    internal class HelpUtil
    {
        public static StringBuilder GetHelpText()
        {
            ...
            var decoderProfileProperties = typeof(SettingsFile.DecoderProfileTemp).GetProperties()
                .Where(Andy.Cmd.Parameter.Metadata.IsParameter)
                .ToList();
            
            var decoderProfileConfigBuilder = new System.Text.StringBuilder();
            Help.PrintParameters<SettingsFile.DecoderProfileTemp, Andy.Cmd.Parameter.ParameterAttribute>(decoderProfileConfigBuilder, decoderProfileProperties, Array.Empty<System.Reflection.PropertyInfo>());
